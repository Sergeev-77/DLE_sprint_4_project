...
def print_gpu_memory():
    ...
    if torch.cuda.is_available():
        for i in range(torch.cuda.device_count()):
            ...
            print(
                f"!max per epoch: {max_allocated:.2f} GB ({max_utilization:.1f}%)"
            )
            print(f"=====>GPU {i}\n")
...
def plot_training_history(train_losses, val_losses, train_maes, val_maes):
    ...
    ax1 = axes[0]
    ax1.plot(
        epochs, train_losses, "b-", linewidth=2, label="train loss", alpha=0.8
    )
    ...
    ax2 = axes[1]
    ax2.plot(
        epochs, train_maes, "b-", linewidth=2, label="train MAE", alpha=0.8
    )
...
def train_one_epoch(
    model, train_loader, val_loader, device, optimizer, criterion, epoch_index=0
):
    model.train()
    epoch_mae = MeanAbsoluteError().to(device)
    batch_mae = MeanAbsoluteError().to(device)
    ...
    val_epoch_mae, val_epoch_loss = validate(
        model, val_loader, epoch_index, criterion, device
    )

    # print_gpu_memory() # отключаем для компактного лога

    print(
        f"{'total epoch ' + str(epoch_index+1):<15}",
        f"train loss {train_epoch_loss:.1f}, val loss {val_epoch_loss:.1f}",
        f"train mae {train_epoch_mae:.1f}, val mae {val_epoch_mae:.1f}",
    )
    print("-" * 40)
...
def train(config, device):
    ...
    for i in range(config.EPOCHS):
        (train_epoch_loss, train_epoch_mae, val_epoch_loss, val_epoch_mae) = (
            train_one_epoch(
                model,
                train_loader,
                val_loader,
...
def check_inference(config, device):
    ...
    def _plot_preds(df, num, best=False):
        ...
        for i in range(num):
            ...
            ax.set_title(
                f"mass: {row['mass']:.0f}\n"
                f"ingredients: {row['ingredients']:.0f}\n"
                f"img: {row['img']}\n"
    ...
    with torch.no_grad():
        for batch in val_loader:
            batch_ingredients = batch["ingr_idxs"]
            batch_mass = batch["mass"]
            inputs = {
                "image": batch["image"].to(device),
                "ingr_idxs": batch_ingredients.to(device),
                "mass": batch_mass.to(device),
            }
            ...
            preds = model(**inputs)
            ingredients.extend(
                [sum(x != 0 for x in i) for i in batch_ingredients]
            )
            img_paths.extend(img_path)
            masses.extend(batch_mass.tolist())
            predictions.extend(preds.cpu().tolist())
            targets.extend(labels.tolist())

    data = list(zip(img_paths, masses, predictions, targets, ingredients))
    df = pd.DataFrame(
        data, columns=["img", "mass", "pred", "target", "ingredients"]
    )
...
def eda(config):
    ...
    for item in range(6):
        ...
        ax.set_title(
            f"mass: {row["initital_mass"]:.0f}\n"
            f"calories: {row["label"]:.0f}\n"
            f"ingredients: {len(row['ingr_idxs']):.0f}",
            fontsize=10,
            ha="center",
        )
        ax.axis("off")
